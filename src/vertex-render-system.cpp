...
VertexRenderSystem::VertexRenderSystem(
    ...
    this->pipeline = std::make_unique<Pipeline>(
        _device, std::vector<VkDescriptorSetLayout>{this->descriptor_manager->generate_layout()},
        std::string("./shaders/ssbo-3d-shader.vert.spv"), std::string("./shaders/ssbo-3d-shader.frag.spv"),
...
void VertexRenderSystem::populate(){
    this->objects->load_file( "models/mcx-spear-eft.obj", {TransformComponent{{10,-10,0},{20,20,20},{0.1,0.4,0.15}}} );
...
void VertexRenderSystem::update_point_light_count(u_int32_t count){
    this->ubo_data.point_light_count = count;
}
...
void VertexRenderSystem::register_descriptors(){
    this->descriptor_manager = std::make_unique<Descriptors>(this->device, this->frame_count);
